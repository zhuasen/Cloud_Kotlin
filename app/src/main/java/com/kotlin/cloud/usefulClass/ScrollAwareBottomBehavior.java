...
public class ScrollAwareBottomBehavior <V extends View> extends CoordinatorLayout.Behavior<V> {
    private int height = 0;
    private int currentState = 2;
    private ViewPropertyAnimator currentAnimator;

    public ScrollAwareBottomBehavior(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    private void animateChildTo(V child, int targetY, TimeInterpolator interpolator) {
        this.currentAnimator = child.animate().translationY((float)targetY).setInterpolator(interpolator).setDuration(225).setListener(new AnimatorListenerAdapter() {
            public void onAnimationEnd(Animator animation) {
                ScrollAwareBottomBehavior.this.currentAnimator = null;
            }
        });
    }
}
