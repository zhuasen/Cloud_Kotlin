...
import com.kotlin.cloudkotlin.oss.ImageAsyncTask
import com.kotlin.cloudkotlin.oss.UserAsyncTask
import com.kotlin.cloudkotlin.usefulClass.GlideImageEngine
import com.kotlin.cloudkotlin.usefulClass.imageUtil
...
class MainActivity : BaseActivity() {
    ...
    companion object{
        ...
        //用户选择图片上传的图片uri
        var imgPathList  = ArrayList<String>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //获取账号
        account = getMySharePref().getString("account", "").toString()
        //欢迎文字
                ...
                selectRequestCode->{
                    ...
                    for (uri in imageListUri) {
                        ...
                        if (imageType == "jpg" || imageType == "jpeg" || imageType == "png") {
                            imgPathList.add(imageUtil.getImagePathFromUri(this, uri))
                        }
                        else "当前仅能上传图片文件,已自动过滤掉视频文件".toastShort()
                    }
/*                    //选择不为空，则通过自己的异步接口上传图片
                    if (imgPathList.isNotEmpty()) ImageAsyncTask(ImageAsyncTask.UPLOAD_FILE_TASK).execute()
                    else  "未上传任何文件".toastShort()*/
                }
            }
        }
    }
